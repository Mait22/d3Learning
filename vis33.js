...
d3.csv("usaRobery.csv")
  .then(data => {
    ...
    const states = d3.keys(returnArray[0]).filter((key)  => { return key !== "date"; });
    ...
    const lineKeys = states.map((name)  => {
      ...
      y.domain([d3.min(lineKeys, (c) => { return d3.min(c.values, (v) => { return v.rate; }); }),
                d3.max(lineKeys, (c) => { return d3.max(c.values, (v) => { return v.rate; }); }) ]);
      x2.domain(x.domain());
      y2.domain(y.domain());
